...
FONT_PATH = os.path.join("fonts", "Genshin_Impact.ttf")
...
# Попытка применить шрифт
try:
    from tkinter import font as tkfont

    # Пробуем зарегистрировать шрифт и получить его имя
    FR_PRIVATE = 0x10
...
video_quality = ctk.StringVar(value="720p")  # создаётся после app!
...
def start_download():
    ...
    def run():
        ...
        # Если это TikTok — заменить на 'best'
...
def on_key(event):
    ...
    # Ctrl + V
    if ctrl and event.keycode == 86:  # V
        try:
            url_entry.insert(tk.INSERT, app.clipboard_get())
        except tk.TclError:
            pass
    ...
    # Ctrl + A
    if ctrl and event.keycode == 65:  # A
